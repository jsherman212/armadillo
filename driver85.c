...
static const char *AD_GET_SYSREG_STRING(unsigned int encoding){
    ...
    case 0xc708: return "CNTKCTL_EL12";
    ...
    case 0x9828: return "DBGDTRRX_EL0";
    case 0x9828: return "DBGDTRTX_EL0";
    ...
    case 0xdf7f: return "PMEVTYPER31_EL0";
    ...
    if(type == AD_OP_REG){
        if(operand.op_reg.sysreg != NONE){
            ...
            if(operand.op_reg.sz != 32 && operand.op_reg.sz != 64){
                printf("'%s'\n", GET_FP_REG(operand.op_reg.rn, operand.op_reg.rtbl));
            }
            else{
                const char *reg = GET_GEN_REG(operand.op_reg.rn, operand.op_reg.zr, operand.op_reg.rtbl);
                printf("'%s'\n", reg);
            }
        }
    }
    else if(type == AD_OP_SHIFT){
        printf("\t\t\tShift type: '%s', amount: %#x\n",
                AD_SHIFT_TABLE[operand.op_shift.type], operand.op_shift.amt);
    }
    else if(type == AD_OP_IMM){
        printf("\t\t\tImmediate type: '%s', value: ", AD_IMM_TYPE_TABLE[operand.op_imm.type]);
        ...
        printf("\t\t\tBase register: '%s', offset: %#x\n", reg, operand.op_imm.off);
...
static void disp_insn(struct ad_insn *insn){
    ...
    printf("\tThis instruction is a(n) '%s' part of group '%s'\n",
